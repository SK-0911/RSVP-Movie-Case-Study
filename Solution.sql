...
-- 2017 had the most number of movies released
...
-- ANS: The year-over-year decrease in the number of movies was observed.
...
-- ANS: 1059 movies were produced in the USA or India in the year 2019
...
-- ANS: Movies that have only one genre is 3289
...
SELECT 
    COUNT(m.id) AS no_of_movie_released, 
    r.median_rating
FROM
    movie AS m
        INNER JOIN
...
(
	SELECT 
		...
		ROUNF(Sum(avg_rating*total_votes)/SUM(total_votes),2) AS actress_avg_rating
	FROM      
		movie AS m
	INNER JOIN 
		ratings AS r
